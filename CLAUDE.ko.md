...
- **파고 기준**은 0.5m 미만 가능 / 1.0m 이하 주의 / 그 이상 불가([swim.ts](lib/engine/swim.ts)).
  ⚠️ 2026-07-21 공식 자료와 대조했고, 그 결과 두 기준의 상태가 갈렸습니다.
  → **파고·수온([파고·수온 ADR](docs/adr/0001-swim-wave-thresholds.md))은 `Accepted`.** 한국은 물놀이객용 고정 파고 컷오프가 없고, 이안류 4단계 지수와 기상 특보로 통제하는데 우리 엔진이 둘 다 이미 씁니다. 유일한 공식 파고 수치(풍랑주의보 3m)보다 우리 1.0m가 훨씬 낮아 **보수적**입니다. 0.5/1.0m는 표준 유래가 아니라 의도적 보수 선택으로 확정했습니다.
  → **갯벌 안전창([갯벌 ADR](docs/adr/0002-mudflat-safety-window.md))은 `Proposed` 유지.** 대조해 보니 ±3시간에 비출 **공식 수치 시간창이 없어서**(해수부 안내는 정성적) 검증할 수가 없습니다. 특히 진입 쪽(간조 −3시간)이 가장 약합니다.
