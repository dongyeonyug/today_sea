...
- Mudflat safety window is low tide ±3h with a return warning 60 min before it closes ([mudflat.ts](lib/engine/mudflat.ts)). Wave thresholds are `<0.5m` 가능 / `≤1.0m` 주의 / else 불가 ([swim.ts](lib/engine/swim.ts)). These were cross-checked against official sources on 2026-07-21 and the two now differ in status: the swim thresholds are **Accepted** ([0001](docs/adr/0001-swim-wave-thresholds.md)) — Korea has no fixed bather wave cutoff; control runs through the 이안류 4-stage index and 기상 특보, our engine already consumes both, and 0.5/1.0m sits well below the only official number (풍랑주의보 3m) so it is conservative by choice. The mudflat window stays **Proposed** ([0002](docs/adr/0002-mudflat-safety-window.md)) — no official numeric window exists to anchor ±3h, so it can't be validated (its 간조−3h entry side is the weakest part). Changing a threshold means updating its ADR.
