...
### 임계값 공식 기준 대조 (2026-07-21 완료)

두 ADR을 공개 공식 자료와 실제로 대조했다. 결과가 갈렸고, 그 갈림 자체가 발견이다.

- [x] [ADR-0001](adr/0001-swim-wave-thresholds.md) — **`Accepted`.** 한국 입욕 통제는
  고정 파고 컷오프가 아니라 **이안류 4단계 지수 + 기상청 풍랑특보 + 지자체 현장
  판단**으로 이뤄짐을 확인. 우리 엔진이 이안류·특보를 이미 소비하고 매핑도 정렬됨.
  유일한 공식 파고 수치(풍랑주의보 유의파고 3m)가 우리 1.0m보다 6배 관대 → 우리 값은
  보수적. 수치는 표준 유래가 아니라 의도적 보수 선택으로 확정. 저체온 20°C와
  해수욕지수 이중계산은 공식 산정식 비공개로 "알려진 한계(수용)"로 남김.
- [ ] [ADR-0002](adr/0002-mudflat-safety-window.md) — **`Proposed` 유지.** 대조 결과
  ±3시간에 비출 **공식 수치 시간창이 없음**(해수부 안내는 정성적: "물 들어오기 전
  복귀, 알람 설정"). 앵커할 표준이 없어 승격 보류. "밀물 2h 전 복귀"는 2차 출처.
  우리 창은 그보다 일찍 닫혀(복귀 쪽) 보수적이나, 진입 쪽(간조 −3h)은 어떤 자료로도
  정당화 안 됨 — 이게 가장 약한 부분. 승격하려면 해수부 직접 문의·사고 시각 통계·
  지점별 갯벌 폭·조위 도입이 필요(각 ADR "남은 검증 항목").

**코드 수치는 바꾸지 않았다** — 대조 결과 우리 값이 공식 기준보다 관대하지 않았으므로
ADR 원칙상 조정 근거가 없다. 엔진 로직 무변경(`verify:verdict` 회귀 없음).
